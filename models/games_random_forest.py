...
──────────────────────────────────────────────────────────────────────────────
Standalone Random Forest Game Model — Chalk Line Labs

Architecture:
  RandomForestClassifier trained on 2015–2023, then isotonic regression
  calibration fitted on the held-out 2024 season.

  Random Forest natively produces compressed probabilities (toward 0.5)
  because it averages votes across trees. Isotonic regression on a held-out
  season corrects this without over-squashing, unlike cross-validation Platt
  scaling which caused compression in the XGBoost model.

Data:
  Shares the mlb_cache_v2/ game log cache with games_research.py and
  games_xgboost.py. No duplicate API calls.
...
CALIB_SEASON     = [2024]                      # 2024 held-out for isotonic calibration
...
RF_PARAMS = {
    'n_estimators':   500,
    'max_depth':      None,       # fully grown trees — RF handles overfitting via bagging
...
# ── Feature engineering (identical to other ML models) ────────────────────────
...
def select_features(matchups):
    cands = ([c for c in matchups.columns if c.startswith('H_')] +
...
def run():
    ...
    if not retrain:
        ...
        if cache.get('trained_year') != CURRENT_YEAR:
            print('New season detected — retraining Random Forest...')
            retrain = True
        else:
    ...
    print(f'✓ Wrote {len(today_preds)} predictions → games-random-forest.json')
