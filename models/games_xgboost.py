...
Data:
  ...
  No duplicate API calls — if research has already run today, all
  historical JSONs are already on disk.
...
TRAIN_SEASONS   = list(range(2015, CURRENT_YEAR))   # all complete seasons
PREDICT_SEASONS = [2025, CURRENT_YEAR] if CURRENT_YEAR > 2025 else [2025]
ALL_SEASONS     = TRAIN_SEASONS + [s for s in PREDICT_SEASONS if s not in TRAIN_SEASONS]
...
# Tuned for standalone performance — more estimators and slightly deeper
# than the ensemble variant since there is no meta-learner to correct errors
XGB_PARAMS = {
    'n_estimators':     600,
    'max_depth':        5,
    'learning_rate':    0.04,
...
# ── Team ID map ───────────────────────────────────────────────────────────────
def get_team_id_map():
    """Reads from shared cache — already populated by research model."""
...
def fetch_game_log(team, year, group, team_ids):
    """
    Reads from the shared mlb_cache_v2/ directory.
    If research model ran first today, historical files are already cached.
    Current season is always re-fetched for fresh rolling stats.
    """
...
def run():
    ...
    if not retrain:
        ...
        if cache.get('trained_year') != CURRENT_YEAR:
            print('New season detected — retraining XGBoost model...')
            retrain = True
        else:
    ...
    if retrain:
        ...
        # Clean — train on all historical seasons
        ...
        print('✓ XGBoost model trained and cached.')
    ...
    print(f'✓ Wrote {len(today_preds)} predictions → games-xgboost.json')
