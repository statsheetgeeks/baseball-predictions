...
// ── Data hook ─────────────────────────────────────────────────────────────────
function useRandomForestData() {
  const [state, setState] = useState({ data: null, loading: true, error: null })
  useEffect(() => {
    fetch('/data/games-random-forest.json')
...
function downloadCSV(predictions, dateStr) {
  ...
  a.href     = url
  a.download = `random_forest_picks_${dateStr.replace(/-/g, '')}.csv`
...
export default function RandomForestGame() {
  const { data, loading, error } = useRandomForestData()
